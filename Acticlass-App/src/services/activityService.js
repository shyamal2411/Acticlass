...
class ActivityService {
  ...
  getNameForActivity(activity) {
    if (
      activity.type == ACTIVITY_TYPES.REQUEST_ACCEPTED ||
      activity.type == ACTIVITY_TYPES.REQUEST_REJECTED
    ) {
      ...
      .then(res => {
        ...
        if (authService.getRole() == ROLES.TEACHER) {
          ...
          for (let activity of res.activities) {
            ...
            data.push([
              date,
              this.getEmailForActivity(activity),
              this.getNameForActivity(activity),
  ...
  generateAndDownloadCSV({group, data, columns}, cb) {
    ...
    if (Platform.OS === 'android') {
      pathToWrite = `${RNFetchBlob.fs.dirs.DownloadDir}/${fileName}`;
    } else {
      pathToWrite = `${RNFetchBlob.fs.dirs.DocumentDir}/${fileName}`;
    }
